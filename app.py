...
# Título
st.markdown('<h1 class="main-title">🏠 Extrator de Matrícula Imobiliária com RAG</h1>', unsafe_allow_html=True)
...
with tab2:
    ...
    if doc_count > 0:
        ...
        st.markdown('<div class="info-box">ℹ️ Nenhum documento no sistema RAG. Processe e adicione documentos na aba "Processar PDF"</div>', unsafe_allow_html=True)
    ...
    if summary["total_documents"] > 0:
        ...
        for doc in documents:
            with st.expander(f"📄 {doc['filename']} (ID: {doc['id']})"):
                ...
                if st.button(f"🗑️ Remover", key=f"remove_{doc['id']}"):
        ...
        # Limpar todos
        st.markdown("### ⚠️ Ações Destrutivas")
        if st.button("🗑️ Remover Todos os Documentos", type="secondary"):
            rag_client.clear_all_documents()
            st.success("Todos os documentos foram removidos!")
            st.rerun()
    
    else:
        st.markdown('<div class="info-box">ℹ️ Nenhum documento no sistema. Processe documentos na aba "Processar PDF"</div>', unsafe_allow_html=True)